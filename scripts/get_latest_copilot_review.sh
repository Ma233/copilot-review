...
if [ -z "$review_json" ] || [ "$review_json" = "null" ]; then
  error "No GitHub Copilot review found for PR #$pr_number"
fi
...
if [ -z "$review_database_id" ]; then
  error "Failed to determine the GitHub review database id for PR #$pr_number"
fi
